@tool
class_name FingerBoneController
...
func _process_modification() -> void:
	...
	if not skeleton:
		return
	_apply(left_grip_bones,     left_controller.get_float("grip")    if left_controller  else 0.0, skeleton)
	_apply(right_grip_bones,    right_controller.get_float("grip")   if right_controller else 0.0, skeleton)
	_apply(left_trigger_bones,  left_controller.get_float("trigger") if left_controller  else 0.0, skeleton)
	_apply(right_trigger_bones, right_controller.get_float("trigger")if right_controller else 0.0, skeleton)
	_apply(left_face_bones,     _face_value(left_controller),  skeleton)
	_apply(right_face_bones,    _face_value(right_controller), skeleton)
